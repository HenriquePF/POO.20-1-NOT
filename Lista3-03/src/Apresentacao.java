...
public class Apresentacao extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		jPanel3Layout.setHorizontalGroup(jPanel3Layout.createParallelGroup(Alignment.LEADING).addGroup(jPanel3Layout
				...
				.addGroup(jPanel3Layout.createParallelGroup(Alignment.LEADING).addGroup(jPanel3Layout
						.createSequentialGroup().addComponent(jLabel7).addGap(18).addComponent(jLabel8)
						.addPreferredGap(ComponentPlacement.RELATED).addComponent(jLabel13).addGap(32)
						.addComponent(jLabel9).addPreferredGap(ComponentPlacement.RELATED).addComponent(jComboBox1,
								GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
						.addGroup(jPanel3Layout.createParallelGroup(Alignment.TRAILING, false)
								.addGroup(jPanel3Layout.createSequentialGroup().addComponent(jBMenorCompromisso)
										.addGap(34).addComponent(jBTempoMedio)
										.addPreferredGap(ComponentPlacement.RELATED, GroupLayout.DEFAULT_SIZE,
												Short.MAX_VALUE)
										.addComponent(jBCompromissosDiaPrioridade))
								.addComponent(jScrollPane2, Alignment.LEADING, GroupLayout.PREFERRED_SIZE, 471,
										GroupLayout.PREFERRED_SIZE)))
				.addGap(293)));
		jPanel3Layout.setVerticalGroup(jPanel3Layout.createParallelGroup(Alignment.LEADING).addGroup(jPanel3Layout
				...
				.addGroup(jPanel3Layout.createParallelGroup(Alignment.BASELINE).addComponent(jLabel7)
						.addComponent(jLabel8).addComponent(jLabel13).addComponent(jLabel9).addComponent(jComboBox1,
								GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE))
				.addGap(9)
				.addGroup(jPanel3Layout.createParallelGroup(Alignment.LEADING)
						.addGroup(jPanel3Layout.createParallelGroup(Alignment.BASELINE).addComponent(jBMenorCompromisso)
	...
	private void jBInserirCompromissoActionPerformed(java.awt.event.ActionEvent evt) {
		LocalDate ld;
		ld = LocalDate.parse(tfDataCompromisso.getText(), formatter);
		DataAgenda dia = agenda.getDataAgenda(ld);
		if (dia == null) {
			JOptionPane.showMessageDialog(this, "Data não está na agenda ainda");
		} else {
			try {
				...
				Compromisso c = new Compromisso(lt, tfDescricao.getText(), Integer.parseInt(tfTempo.getText()),
						prioridade);
				dia.addCompromisso(c);
				JOptionPane.showMessageDialog(this, "Compromisso agendado");
	...
	private void jBIncluirNaAgendaActionPerformed(java.awt.event.ActionEvent evt) {
		...
		String str = "";
		try {
			ld = LocalDate.parse(tfData.getText(), formatter);
			DataAgenda dia = agenda.getDataAgenda(ld);
			if (dia != null) {
				str = "Data já está na agenda, com " + dia.getQtdCompromissosPrioridade('A')
						+ " compromissos de alta prioridade.\n";
				...
				dia = new DataAgenda();
				dia.setData(ld);
				dia.setEfemeride(tfEfemeride.getText());
				agenda.addDataAgenda(dia);
				str = "Data incluída na agenda";
			}
		} catch (IllegalArgumentException iae) {
			str = iae.getMessage();
		}
